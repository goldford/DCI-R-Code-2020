...
get_segments_distance <- function(network = NULL){
  ...
  for(seg.from in seg.copy){
    ...
    # NOTE might be better to move this and then add in the 0 distances between same pairs later, unsure
    # Remove first element to not repeat inverse distance calculations
    seg.copy <- seg.copy[-1]  
